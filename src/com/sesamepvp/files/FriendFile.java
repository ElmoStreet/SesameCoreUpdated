...
public class FriendFile {
	static FriendFile instance = new FriendFile();
	Plugin p;
	...
	public static FriendFile getInstance() {
		return instance;
	}
//commit 2
	public void setup(Plugin p) {
		...
		dfile = new File(p.getDataFolder(), "friends.yml");
		if (!dfile.exists()) {
			try {
				dfile.createNewFile();
			} catch (IOException e) {
				Bukkit.getServer().getLogger()
						.severe(ChatColor.RED + "Could not create friends.yml!");
			}
		}
	...
	public void saveData() {
		try {
			...
			Bukkit.getServer().getLogger()
					.severe(ChatColor.RED + "Could not save friends.yml!");
		}
	}
